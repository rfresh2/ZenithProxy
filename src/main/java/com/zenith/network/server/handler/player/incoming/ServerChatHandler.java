...
//                PUnsafe.putObject(packet, CLIENTCHATPACKET_MESSAGE_OFFSET, message.substring(1));
                ...
                if (args.size() == 1) {
                    ...
                    session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cAll Spectators kicked§r", true));
                } else {
                    ...
                    session.getProxy().getSpectatorConnections().stream()
                            ...
                            .forEach(connection -> {
                                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cKicked " + playerName + "§r", true));
                                connection.disconnect(CONFIG.server.extra.whitelist.kickmsg);
                            });
                }
                return false;
                ...
                saveConfig();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cSpectators public chat toggled " + (CONFIG.server.spectator.spectatorPublicChatEnabled ? "on" : "off") + "§r", true));
                return false;
                ...
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cSpectators toggled " + (CONFIG.server.spectator.allowSpectator ? "on" : "off") + "§r", true));
                return false;
            } else if (lowerCase.startsWith("!sync")) {
                PlayerCache.sync();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cSync inventory complete", true));
                return false;
            } else if (lowerCase.startsWith("!chunksync")) {
                ChunkCache.sync();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cSync chunks complete", true));
                return false;
            } else {
                session.send(new ServerChatPacket(String.format("§7[§9ZenithProxy§7]§r §cUnknown command: §o%s", message), true));
                return false;
            }
            ...
            if (lowerCase.startsWith("/ignorelist")) {
                ...
                // todo
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cNot implemented yet", true));
                return false;
                ...
                if (split.length == 2) {
                    ...
                    if (WHITELIST_MANAGER.isPlayerIgnored(player)) {
                        ...
                        session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cRemoved " + player + " from ignore list", true));
                        return false;
                    }
                    WHITELIST_MANAGER.addIgnoreWhitelistEntryByUsername(player).ifPresentOrElse(
                            ignoreEntry -> session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cAdded " + ignoreEntry.username + " to ignore list", true)),
                            () -> session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cFailed to add " + player + " to ignore list", true))
                    );
                } else {
                    session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cInvalid syntax. Usage: /ignore <name>", true));
                }
                return false;
            } else if (lowerCase.startsWith("/togglechat")) {
                CONFIG.client.extra.chat.hideChat = !CONFIG.client.extra.chat.hideChat;
                saveConfig();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cChat toggled " + (CONFIG.client.extra.chat.hideChat ? "off" : "on") + "§r", true));
                return false;
                ...
                saveConfig();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cWhispers messages toggled " + (CONFIG.client.extra.chat.hideWhispers ? "off" : "on") + "§r", true));
                return false;
                ...
                saveConfig();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cDeath messages toggled " + (CONFIG.client.extra.chat.hideDeathMessages ? "off" : "on") + "§r", true));
                return false;
                ...
                saveConfig();
                session.send(new ServerChatPacket("§7[§9ZenithProxy§7]§r §cConnection messages toggled " + (CONFIG.client.extra.chat.showConnectionMessages ? "on" : "off") + "§r", true));
                return false;
            }
        }
        return true;
